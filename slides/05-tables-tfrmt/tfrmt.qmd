...
First, we only need the counts themselves, not denominators or percentages. Second, we need to give these a unique stat name to distinguish them for the `big_n_structure` in {tfrmt}. 
...
::: {.small} 
We can use the `prep_big_n()` function for this:
:::
```{r}
