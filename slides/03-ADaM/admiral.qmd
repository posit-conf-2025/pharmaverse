...
-   I will walk you through certain parts of the  `ADSL` and `ADVS` R scripts
    - Let's take a quick peek at [scripts](https://github.com/posit-conf-2025/pharmaverse/tree/main/slides/03-ADaM/scripts).
    -   Discussion on **certain** functions and function arguments from [scripts](https://github.com/posit-conf-2025/pharmaverse/tree/main/slides/03-ADaM/scripts) for deriving variables/records. 
...
-   Brief notes on the code in script and exercises:
    ...
    -   Exercises will need some lite updates.
    -   3 folks not presenting can help diagnose issues. 
:::
...
- P21-like spec
    - P21 is spec/data validation tool widely used in the industry for creating SDTMs
    and ADaMs
...
* Note: Referencing lines in file at each section (ADSL Lines: ##-##)
...
## Reading in our spec for ADSL (ADSL Lines: )
...
## Combine Parent and Supplementary Data (ADSL Lines: )
...
## Let's turn a `--DTC` to a `*DTM` variable (ADSL Lines: )
...
* Filter, join and rename is a common pattern in ADaMs.
...
## Let's derive a Duration Variable (ADSL Lines: )
...
## Let's apply Control Terms / Code Lists (ADSL Lines: )
...
* We are going to update the function `derive_vars_dtm()`
